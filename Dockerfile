...
ENTRYPOINT ["/bin/sh", "-c","java -jar /rocketmq-test-tools.jar -testRepo=${2} -action=${3} -version=${4} -askConfig=${5} -velauxUsername=${6} -velauxPassword=${7} -chartGit=${8} -chartBranch=${9} -chartPath=${10} -testCodeGit=${11} -testCodeBranch=${12} -testCodePath=${13} -jobIndex=${14} -helmValue=${15}"]
